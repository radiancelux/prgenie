...
Listen loops cap at **60 ticks** (~1 hour), then halt. Re-run `/watch-review-inbox` or `/watch-ready-prs` to continue. `/stop-loop` (implementor), `/stop-review` (reviewer), and `/stop-watch` (both) stop sooner.
