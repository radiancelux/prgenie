...
- Command (skills, one slash name each): `/loop`, `/start`, `/local-pr`, `/review`, `/watch-ready`, `/watch-inbox`, `/queue`, `/inbox`, `/stop`, `/stop-review`, `/unwatch`, `/export`
...
Preferred orchestration is **`/loop`** (one steward, durable Task ids, export gate before Push to origin). Listen loops are transitional: **idle timeout** (default **30m** quiet, **8h** wall max), then halt that chat's lane only (`/stop` or `/stop-review`). Re-run `/watch-inbox` or `/watch-ready` to start that lane again. `/unwatch` stops both. TICK is fingerprint-change only — do not re-arm listen on TICK. Reviewer Tasks require a durable `claim_review` for that `id`+`headSha`.
