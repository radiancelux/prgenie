...
If you are **starting** work (no loop yet), follow `/start-loop` instead: take a ticket or a chat brief, then `create_local_pr`. That leaves `main` for a feature branch.

If a local PR already exists for this branch (and is not archived/`approved`), show it with `list_local_prs` and update via `update_local_pr` (summary), `set_status`, or `add_comment` instead of opening GitHub. If it is `changes_requested`, read `pendingComments` first (`/review-inbox` or `/watch-review-inbox`) and `address_comment` each. If it is still `ready`, the reviewer is still writing — do not address comments yet. Fill or refresh `body` before marking ready.
