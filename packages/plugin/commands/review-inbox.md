...
0. Halt check — do not skip if MCP `watch_status` is missing. Prefer `node packages/cli/dist/prgenie.cjs watch`. `listening` continues. `halted reason=stop` → kill the listen loop. Do not implement. `halted reason=export` → that packet shipped (or export is in flight). Do **not** `prgenie watch start` if this checkout's live loop **is** the export id. `create_local_pr` resumes after that id is archived. If a **different** live loop is on this checkout, `prgenie watch start` and continue.
