...
2. If there are no `pendingComments`, say so in one line. If status is still `changes_requested` and everything is resolved, `set_status` `ready` and `add_comment` `role=agent` **Review requested.** Do not invent new work.
3. Skip ids you already submitted for a second review this session **unless** new unresolved comments arrived.
4. `get_local_pr` — `pendingComments` is the brief (each has `id`).
5. Fix on the current branch. Commit if needed.
6. For each pending comment: MCP `resolve_comment` with `commentId` and a reply of what you changed. That marks it resolved. Do not only post a loose `role=agent` comment.
