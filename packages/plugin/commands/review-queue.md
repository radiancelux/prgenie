...
3. For each remaining loop, Task `generalPurpose` **in parallel** (one Task per loop). Prompt: review that id — `get_local_pr`, `get_diff`, read `body`, `add_comment` `role=reviewer` for new findings, `resolve_comment` addressed threads that are fixed, or `complete_review` if nothing else is wrong (sets `reviewed` for the human). Do not implement or git push.
