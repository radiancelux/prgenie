...
function refusePrimaryWorktreeIfParallel(worktreePath, primary, loopId, liveLoopIds) {
  const others = [...liveLoopIds].map((id) => id.toLowerCase()).filter((id) => id !== loopId.toLowerCase());
...
function sameFsPath(a, b) {
  try {
    const leftStat = (0, import_node_fs2.statSync)(a);
    const rightStat = (0, import_node_fs2.statSync)(b);
...
async function ensureLoopFeatureBranch(cwd, options) {
  ...
  if (here && isBaseBranch(current, options.baseRef)) {
    const created2 = await git(cwd, ["branch", headRef], { allowFail: true });
    if (created2.code !== 0 && !await branchExists(cwd, headRef)) {
      throw new Error(`Could not create loop branch ${headRef}: ${created2.stderr.trim()}`);
    }
...
function peelStashMessage(loopId) {
  return `prgenie-exclusive-peel-${loopId}`;
}
async function findPeelStashRef(cwd, loopId) {
...
async function freePrimaryFromLoopBranch(primary, loop) {
  const branch = await currentBranch(primary);
  if (branch !== loop.headRef) return false;
  let stashed = false;
...
async function restorePeelStash(dest, loopId) {
...
async function completeLocalPrReview(cwd, id, options = {}) {
  ...
  return withFileLock(file, async () => {
    ...
    pr.comments.push({
      id: newId2("c"),
      body: (options.body?.trim() || (handedToImplementor ? "Review complete. Findings are ready for the implementor." : "Review cleared. Steward will run the export gate.")).trim(),
      createdAt: now,
      author,
      role: "reviewer",
      status: "resolved"
...
function emptyCiProgressSnapshot() {
...
function applyCiProgressEvent(current, event) {
  ...
  const ensure = (name) => {
    ...
    const created = { name, state: "queued" };
    checks.push(created);
    return created;
  };
...
function formatProgressCard(snapshot) {
  ...
  for (const name of names) {
    ...
    const extra = state === "fail" && row?.message ? ` \u2014 ${row.message}` : state === "start" && row?.command ? ` (${row.command})` : "";
...
function requestCiAbort(cwd, id) {
  ...
  (0, import_node_fs3.writeFileSync)(file, `${JSON.stringify(token)}
`, "utf8");
  return next;
}
...
async function acquireCiLock(cwd, id, headSha, signal) {
  ...
  for (; ; ) {
    ...
    try {
      const fd = (0, import_node_fs3.openSync)(file, "wx");
      try {
...
function classifyCiPath(filePath) {
  ...
  if (/\.(md|mdc|txt)$/.test(lower) || p.startsWith("docs/") || /^(readme|license|changelog|authors|roadmap)(\.|$)/i.test(base)) {
    return "docs";
  }
...
function packageFromScopedCheck(check) {
...
function fullSuite(reasons, paths, uncertain) {
  ...
  const mapping = DEFAULT_CI_CHECKS.map((check) => ({
    check,
    reason: reason.join("; ")
  }));
  return {
...
function selectCiChecks(changedPaths) {
  ...
  const mapping = checks.map((check) => {
...
async function checkFormatFromBlobs(cwd, files, signal) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const ok2 = await prettier.check(source, {
        ...config ?? {},
        filepath
      });
      if (!ok2) failures.push(file);
    } catch (err) {
...
async function runOneCheck(cwd, check, options) {
...
async function runCiChecks(cwd, options = {}) {
  ...
  const parallel = selection?.packageScoped === true ? false : options.parallel ?? envFlag("PRGENIE_CI_PARALLEL", true);
...
async function handleTool(name, args) {
  ...
  switch (name) {
    ...
    case "run_ci": {
      const failing = typeof args.failingChecks === "string" ? args.failingChecks.split(",").map((s) => s.trim()).filter(Boolean) : Array.isArray(args.failingChecks) ? args.failingChecks.filter((s) => typeof s === "string") : [];
      const card = createProgressCardSink((line) => process.stderr.write(`${line}
`));
      const result = await runLoopCi(cwd, String(args.id ?? ""), {
        failingChecks: failing,
        failFast: args.failFast === false ? false : void 0,
        parallel: args.parallel === false ? false : void 0,
...
`));
      ...
      return { ...result, progressCard: card.snapshot().checks.length ? card.card() : void 0 };
    }
    default:
...
var tools = [
  ...
  },
  {
    ...
    description: "Ensure this loop has a git worktree and return its path. Creates a sibling <repo>.loops/<id> checkout (exclusive; never the primary folder).",
    inputSchema: {
      type: "object",
      required: ["id"],
  ...
  },
  {
    ...
    description: "Reviewer or human: mark an addressed finding resolved and attach a reply under it. If nothing open or addressed remains, the loop becomes reviewed (review cleared; steward runs the export gate \u2014 not a human handoff). Do not git push.",
    inputSchema: {
      type: "object",
      required: ["id", "commentId", "body"],
      properties: {
  ...
  {
    name: "run_preflight",
    description: "Run pattern-memory preflight on a local PR to see if any learned patterns would be matched. This is automatically run when set_status ready unless skipPreflight is set. Returns passed boolean and issues array. Distinct from run_ci (local format/lint/test).",
    inputSchema: {
      type: "object",
      required: ["id"],
      properties: { id: { type: "string" }, cwd: { type: "string" } }
