...
description: PR Genie loop implementor (strong tier). Use only when /steward spawns the implementor for a local PR loop, and only when the steward bumps a design-heavy or repeatedly failing AC. Never for review.
