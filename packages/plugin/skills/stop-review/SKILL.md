---
name: stop-review
description: Stop the reviewer listen loop. Halts only the queue lane, kills the /queue shell, and leaves the implementor inbox running. Use when the user runs /stop-review or wants the reviewer chat to stop listening.
...
2. Kill the monitored shell / `/loop` you started for `/queue` (`AGENT_LOOP_TICK_review-queue`). Await that shell so a later completion notice does not re-arm you. A TICK is not a reason to start listen again.
...
5. To publish, they must run `/export`. To listen again, they run `/watch-ready`. Queue stop never auto-resumes.
