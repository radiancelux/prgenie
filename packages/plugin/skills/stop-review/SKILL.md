...
2. Kill the monitored shell / `/loop` you started for `/review-queue` (`AGENT_LOOP_TICK_review-queue`). Await that shell so a later completion notice does not re-arm you. A TICK is not a reason to start listen again.
