...
4. If there are no `pendingComments`, say so in one line. If status is still `changes_requested` with everything **addressed** (legacy packet), `set_status` `ready`. Do not invent new work.
...
7. Fix on the current branch. Commit if needed **before** addressing so HEAD is current.
