...
4. Post **all** new findings with `add_comment` `role=reviewer` (HIGH/MEDIUM only). Status stays `ready`. Long findings: MCP `add_comment` (Content-Length framed) or `prgenie comment --body-file`. Do not pass finding text through an unquoted shell `-m`.
