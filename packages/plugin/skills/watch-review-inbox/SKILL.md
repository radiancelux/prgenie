...
Start `/watch-review-inbox`. If there is no live loop on this worktree, `/start-loop` first (ticket or chat brief) — that resumes an **export** halt. `/stop-watch` stays halted. Then one `/review-inbox` now, `/loop 1m /review-inbox`. Only this checkout's loop. Only implement when status is `changes_requested`. Address each open finding with `address_comment` (commit first). The last address sets `ready` and posts Review requested.
