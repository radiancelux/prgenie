...
Start `/watch-review-inbox`: one `/review-inbox` now, then `/loop 1m /review-inbox`. `pendingComments` is the brief. Address each with `address_comment`, then `ready` + `Review requested.`
