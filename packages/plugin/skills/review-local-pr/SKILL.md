...
2. `get_local_pr` / `prgenie show` + `get_diff`. If `reviewRequestedSha` is null, call `mark_review_requested` / `prgenie` equivalent (MCP: not always listed — use `node -e` or ensure via `set_status ready` which arms it) so the drift guard has a baseline — or record the `headSha` you are about to review and treat any later mismatch as drift. **Large loops:** call `get_diff` with `stat=true` (or `prgenie diff --stat`) first; if the full diff would truncate (~80KB on MCP), call `get_diff` again with `paths` for the files you need. Read `body`. `addressedComments` means a **second review** — verify those replies against the diff. Do not re-file a finding that is actually fixed.
