...
0. Halt check — do not skip if MCP `watch_status` is missing. Prefer `node packages/cli/dist/prgenie.cjs watch`. `listening` continues. `halted reason=stop` → kill the listen loop. `halted reason=export` → last packet shipped; do not dispatch. A new `create_local_pr` resumes watch only after that export id is archived or missing.
