...
- **Git fixture templates** (RAD-133): heavy core suites (`export-gate`, `shepherd`, `ci-runner` loop-CI, `ci-abort`) clone a process-local seeded template (`packages/core/src/test-git-fixture.ts`) instead of `git init` per test. Templates are removed on process exit (best effort). Measured Windows solo `export-gate.test.ts` (2026-09-25): `main` @ `d9f5674` mean **104.8s**; this PR mean **113.2s** (**103.2s** uncontended). The ~522s dogfood baseline did not reproduce solo — wall time is dominated by six tests that run real CI checks. AC5 speed target moved to **RAD-146**.
