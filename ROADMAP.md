...
| # | Gap | Evidence |
| --- | --- | --- |
| H1 | ~~No way to see or control watch lanes from the sidebar.~~ **Historical:** inbox/queue Start/Stop shipped, then **removed with the listen flywheel (RAD-81)**. Export-halt diagnostics only. | Was `laneView.ts` + watch APIs; now hard-error to `/loop`. |
...
| # | Gap | Evidence |
| --- | --- | --- |
| A1 | ~~Hand-rolled PowerShell listen loops~~ **Historical:** `prgenie watch listen` shipped, then **removed (RAD-81)**. Steward `/loop` is current. | Skills call steward MCP; no listen ticks. |
...
- **[RAD-83](https://linear.app/radiancelux/issue/RAD-83/dogfood-ux-export-emptydraft-layout-loop-rename-mcp-cwd)** — Dogfood UX: EXPORT empty/draft layout + `/loop` rename + MCP cwd (Medium · Backlog). **Note:** MCP cwd shipped via [RAD-86](https://linear.app/radiancelux/issue/RAD-86/mcp-default-cwd-to-workspacegit-root-no-silent-not-a-git-repo); EXPORT UX + slash rename remain.
...
- **[RAD-85](https://linear.app/radiancelux/issue/RAD-85/docs-one-page-human-happy-path-install-loop-export)** — Docs: one-page human happy path (install → `/loop` → export) (Medium · Backlog)
